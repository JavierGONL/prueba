...
% Usa STIX Math que tiene los símbolos geométricos
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{{imagenes/diagrama mecanismo}.png}
...
\begin{enumerate}
    ...
    \item Y - $O_1$:
    \begin{equation}
        O_{1y} + A\cos\theta - B_y - W_{o_1} = m_{o_1}(\bar{a}_{o_1})_y \quad \Rightarrow \quad O_{1y} + A\cos\theta = m_{o_1}(g + \bar{a}_{o_1y})+ B_y
    \end{equation}
...
Se presentan únicamente las tres curvas clave que sintetizan el desempeño del mecanismo: posición vertical del martillo, aceleración vertical y par motor requerido. Estas permiten evaluar funcionalidad (carrera y asimetría), exigencia dinámica (picos de aceleración) y demanda energética (variación de par).

\subsection*{Posición vertical del martillo}
Evolución $y_c(t)$: carrera y asimetría (avance lento, retorno rápido).
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{{salidas/yc_vs_t}.png}
    \caption{Posición vertical $y_c(t)$ del martillo. Con los parámetros actuales: recorrido total $\Delta y \approx 0.280\,\text{m}$ (280 mm).}
    \label{fig:yc_ciclo}
\end{figure}
\noindent Con estos parámetros, $y_{\min} \approx -0.140\,\text{m}$ y $y_{\max} \approx 0.140\,\text{m}$; el descenso dura $\approx 0.398\,\text{s}$ y el retorno $\approx 1.17\,\text{s}$ (relación $\approx 2.95$).

\subsection*{Aceleración vertical}
Los picos de $\ddot{y}_c$ determinan las fuerzas inerciales máximas transmitidas a las uniones y soportes; son críticos para el dimensionamiento estructural.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{{salidas/ac_vs_t}.png}
    \caption{Aceleración vertical $\ddot{y}_c$ del martillo.}
    \label{fig:ac_ciclo}
\end{figure}

\subsection*{Par motor requerido}
La variación del par $\tau$ evidencia los picos de demanda energética asociados a cambios rápidos de estado cinemático. Estos valores guían la selección del motor y justifican (si las oscilaciones son grandes) el uso de un volante de inercia.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{{salidas/tau_vs_t}.png}
    \caption{Par motor instantáneo $\tau$ necesario para mantener la velocidad de entrada.}
    \label{fig:tau_ciclo}
\end{figure}

\subsection*{Síntesis}
\begin{itemize}
    \item La curva $y_c(t)$ confirma una carrera estable y la asimetría funcional del retorno rápido.
    \item Los picos de $\ddot{y}_c$ delimitan el rango de esfuerzos dinámicos a considerar en soporte y guiado.
    \item La variación de $\tau(t)$ establece la potencia intermitente y la posible necesidad de almacenamiento de energía (volante) para suavizar demanda.
\end{itemize}
