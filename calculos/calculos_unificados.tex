...
Elevando ambas ecuaciones al cuadrado y sumandolas:
...
\begin{itemize}
	...
	\item $\vec v_C = \dot{y}_c$ ↓
\end{itemize}

[diagrama velocidades]
...
donde:
\begin{itemize}
	\item $\vec a_B = (\vec a_B)_t + (\vec a_B)_n$ con:
	\begin{itemize}
		\item $(\vec a_B)_t = \alpha_1 \cdot R$, ⦭ $\theta$
		\item $(\vec a_B)_n= \omega_1^2 \cdot R$ ⦫ $\theta$
	\end{itemize}
	\item $\vec a_{C/B} = (\vec a_{C/B})_t + (\vec a_{C/B})_n$ con:
	\begin{itemize}
		\item $(\vec a_{C/B})_t = \alpha_3 \cdot K$ ⦪ $\beta$
		\item $(\vec a_{C/B})_n = \omega_3^2 \cdot K$ ⦯ $\beta$
	\end{itemize}
	\item $\vec a_C = \ddot{y}_c$ ↓ 
\end{itemize}
