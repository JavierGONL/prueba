...
\begin{align}
\bar{a}_{O_1} &= \alpha_1 \lambda ,\;⦭ \, \theta^\circ \quad + \omega_1^2 \lambda ,\; ⦫\, \theta^\circ \\
\bar{a}_{O_{1x}} &= -\left(\alpha_1 \lambda \sin\theta + \omega_1^2 \lambda \cos\theta\right) \\
...
\end{enumerate}
\subsection{Potencia y eficiencia del mecanismo}

Conociendo tanto las variables cinematicas como cineticas del mecanismo es posible obtener expresiones para la potencia de entrada y salida:
\begin{align}
P_{in} &= \tau \cdot \omega_2 \\
P_{out} &= m_c a_c \cdot v_{c}
\end{align}
A su vez con los valores de potencia es posible encontrar la eficiencia del mecanismo por medio de la siguiente proporcion:
...
\section{solucion cinetico del sistema dado una fuerza de entrada}

Dado un valor requerido de fuerza vertical en el martillo $C$, el objetivo es obtener las reacciones internas y el par del motor $\tau$, reduciendo las ecuaciones de cinetica


\[
\begin{aligned}
O_{1x} - A\sin\theta &= m_{o1}\,\bar a_{o1x} + B_x,\\[2pt]
O_{1y} + A\cos\theta &= m_{o1}\,(\bar a_{o1y} + g) + B_y,\\[2pt]
\lambda\sin\theta\,O_{1x} - \lambda\cos\theta\,O_{1y} + (L-\lambda)A &= \bar I_{o1}\,\alpha_1 - \lambda\big(B_x\sin\theta - B_y\cos\theta\big),\\[2pt]
-2\cos(\phi-\theta)\,A + \frac{2}{r}\,\tau &= \frac{2}{r}\,\bar I_{o2}\,\alpha_2 + m_{o2}\,\big(\bar a_{o2y} + g - \bar a_{o2x}\big).
\end{aligned}
\]

Con la fuerza necesaria en el martillo $C$ y la cinemática instantánea se determina primero $(B_x,B_y)$, luego $(O_{1x},O_{1y},A)$ y finalmente el par requerido $\tau$. 
